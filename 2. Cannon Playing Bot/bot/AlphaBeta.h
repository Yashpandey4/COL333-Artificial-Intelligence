...
int cannons(GameMatrix *state)
{
    ...
    return 60*(cannonkills1-cannonkills2)+ (cannoncount[0]+cannoncount[1]+cannoncount[2]) + 0*(backcan[0]+backcan[1]+backcan[2]) + (sidecan[0]+sidecan[1])
                                         -(cannonbad[0]+cannonbad[1]+cannonbad[2]) + 0*(backcanbad[0]+backcan[1]+backcan[2]) -(badsidecan[0]+badsidecan[1]);
}
...
{ 
    ...
    if(state->MeNoOfTownhalls==2 || state->OpNoOfTownhalls == 2)
    {
        ...
        {
            return (-1*(state->MeNoOfTownhalls-state->OpNoOfTownhalls)*4 + 4)*(-6000);
        }
        else if(state->OpNoOfTownhalls==2 )
        {
            return ((state->MeNoOfTownhalls-state->OpNoOfTownhalls)*4 + 4)*6000;
        }
    }
    if(state->MeNoOfSoldiers==0 || state->OpNoOfSoldiers == 0)
    {
        ...
        {
            return (-1*(state->MeNoOfTownhalls-state->OpNoOfTownhalls)*4 + 6)*(-6000);
        }
        if(state->OpNoOfSoldiers==0)
        {
            return ((state->MeNoOfTownhalls-state->OpNoOfTownhalls)*4 + 6)*(6000);
        }
    }
    if(d==0)
    {
    ...
    if(state->childStates.size()==0)
    {
        ...
        return ((state->MeNoOfTownhalls-state->OpNoOfTownhalls)*4-2)*(6000);
    }
    ///////////////////
    int v = INT32_MIN;
    int temp = 0;
